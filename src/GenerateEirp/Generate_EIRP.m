...
%% %%%%%%%%%%%%%%%%%%%%%%%    Antenna Element   %%%%%%%%%%%%%%%%%%%%%%%%%%%%
antennaEleTx            = phased.NRAntennaElement( ...
    'FrequencyRange', FreqBans, ...
    'Beamwidth', [HPBW_H, HPBW_V], ...
    'PolarizationModel', Polarization, ...
    'MaximumGain',AnElementGain ...
);

%% %%%%%%%%%%%%%%%%%%%%%%% Antenna Array pannel %%%%%%%%%%%%%%%%%%%%%%%%%
AntArrayTx              = phased.NRRectangularPanelArray( ...
    'ElementSet', repmat({antennaEleTx}, 1, Polarization), ...
    'Size', [SubArraySize, ArraySize], ...
    'Spacing', [AnElementSpacing_V, AnElementSpacing_H, SubArraySpacing_V, SubArraySpacing_H] ...
);

SteerVecTx_Array        = phased.SteeringVector( ...
    'SensorArray', AntArrayTx, ...
...
SSB_AntArrayTx              = phased.NRRectangularPanelArray( ...
    'ElementSet', repmat({antennaEleTx}, 1, Polarization), ...
    'Size', [SubArraySize, ArraySize],...
    'Spacing', [AnElementSpacing_V, AnElementSpacing_H, SubArraySpacing_V, SubArraySpacing_H]);

SSB_SteerVecTx_Array        = phased.SteeringVector( ...
    'SensorArray', SSB_AntArrayTx, ...
    'PropagationSpeed', PropSpeed, ...
    'IncludeElementResponse', true);
...
[azSSB.sweepBW, ...
    ...
    azSSB.steerAngle, ...
    elSSB.steerAngle, ...
    elSSB.steerAngleMat] = azPlainSSBsteerAngle( ...
        azSweepRangeSect, ...
        CoarseConfSSbeams, ...
        elSweepCoarseSSB ...
    );
...
fprintf('Element spacing (V, H): %.3f m (%.2f lambda), %.3f m (%.2f lambda)\n', ...
    AnElementSpacing_V, AnElementSpacing_V / lambda, ...
    AnElementSpacing_H, AnElementSpacing_H / lambda);
...
for iissb = 1:numSSbeams
    ...
    SSB_gNB_WT_AntArrayPanel = SSB_SteerVecTx_Array( ...
        CenterFreq, ...
        [azSSB.steerAngleMat(iissb); elSSB.steerAngleMat(iissb)]);   % It steers toward azimuth and elevation angle, i.e., (Az,El).

    [EIRP_ElAz_SSB_dB, AngleAxisAz_SSB, AngleAxisEl_SSB] = pattern( ...
        SSB_AntArrayTx, ...
        CenterFreq, ...
        -180:AzUpSam:180, ...
        -90:ElUpSam:90, ...
        ...
        'Type', 'powerdB', ...
        'Weights', SSB_gNB_WT_AntArrayPanel .* SSBactiveAntEleInd, ...
        'Normalize', true);

    [AF_ElAz_SSB_eField, AngleAxisAz_SSB, AngleAxisEl_SSB] = pattern( ...
        SSB_AntArrayTx, ...
        CenterFreq, ...
        -180:AzUpSam:180 ...
        ,-90:ElUpSam:90, ...
