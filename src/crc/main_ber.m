%% ------------------------------------------------------------------------
% Author  : Joshua (derived from Armed Tusha original)
% Institution : University of Notre Dame
% Date        : October 24, 2025 (Modified)
% ------------------------------------------------------------------------
% This script is based on `code5_code4selectSnr.m` and is intended to run
% on headless / non-GUI MATLAB environments (HPC). It records the simulation
% start and end times, prints progress to the command window, and saves the
% requested simulation parameters and results to a .mat file (v7.3).
%
% Run on an HPC with: matlab -nodisplay -nosplash -r "run('code6_code5crc.m'); exit"
% ------------------------------------------------------------------------
...
%% csr-rs config
csirs = nrCSIRSConfig;
csirs.CSIRSType           = {'nzp'};        % One NZP resource is fine
csirs.RowNumber           = 8;              % 8 ports
csirs.NumRB               = 52;
csirs.RBOffset            = 0;
csirs.CSIRSPeriod         = [4 0];
csirs.SymbolLocations     = {[2]};
csirs.SubcarrierLocations = [2 6];         % 6 k_i per PRB
csirs.Density             = {'one'};

%% report config
reportConfig.CodebookType   = 'Type1SinglePanel';
reportConfig.PanelDimensions = [2 2];  % 8-port (dual-pol 2x2)
...
%% more params
nTxAnts                     = csirs.NumCSIRSPorts;    % Number of transmit antennas (physical)
nRxAnts                     = 4;                         % Number of receive antennas (physical)
...
% Start diary to capture command-window output to the text file
try
    diary(saveFileTxt);
    diary on;
...
fprintf('Number of iterations (packets) per link configuration: %d\n', numIter);
fprintf('Modulation Orders: %s\n', strjoin(ModOrderList, ', '));
fprintf('SNR Ranges (dB):\n');
fprintf('  16QAM: %s\n', mat2str(SNRdB_16QAM));
fprintf('  64QAM: %s\n', mat2str(SNRdB_64QAM));
fprintf('  256QAM: %s\n', mat2str(SNRdB_256QAM));
fprintf('Precoding Types: %s\n', strjoin(string(pmiPrecodingList), ', '));
fprintf('Perfect Estimation: %s\n', string(perfectEstimation));
fprintf('Coding Rate: %.4f\n', codeRate);
            ...
            for isi = 1:numSNR
                ...
                for nslot = 0:noSlotsSim-1
                    ...
                    [~, pmiPractical, ~, pmiInfoPractical] = hCQISelect(localCarrier, csirs, ...
                                reportConfig, pdsch.NumLayers, PracticalHest, nVarPractical);
...
%% ------------------------------------------------------------------------
% Record end time and save results (HPC-safe)
% -------------------------------------------------------------------------
...
try
    save(saveFile, 'noSlotsSim','ModOrderList','pmiPrecodingList', ...
        'SNRdB_16QAM','SNRdB_64QAM','SNRdB_256QAM','perfectEstimation','numIter', ...
        'MaximumDopplerShift','DelaySpread', 'nLayers', 'csirsPorts', ...
        'carrier','NHARQProcesses','codeRate','encodeDLSCH_template','decodeDLSCH_template', ...
        'csirs','reportConfig','nRxAnts','startTime','endTime', ...
        'avgBER_all','totalPacketErrors_all','avgBitErrorsPerPacket_all','SNRdB_all','combinationLabels', ...
        '-v7.3');
...
% Plotting removed for headless/HPC runs.
