...
%% -------------------------------------------------------------------------
% OPTIMIZED Monte Carlo Simulator with Packet Error Tracking
% -------------------------------------------------------------------------
...
afterEach(D, @(x) fprintf('Iteration %d/%d (%.1f%%) - SNR: %.1f dB, BER: %.2e, PktErr: %d\n', ...
    x.iter, numIter, 100*x.iter/numIter, x.snr, x.ber, x.pktErr));
...
parfor msnr = 1:numIter
    ...
    if mod(msnr, progressStep) == 0
        progressData = struct('iter', msnr, 'snr', SNRdB(1), 'ber', ber_snr(1), 'pktErr', packetError_snr(1));
        send(D, progressData);
    end
end
...
% Calculate average bit errors per packet for each SNR
...
% -------------------------------------------------------------------------
figure
semilogy(SNRdB, avgBER, '-o', 'LineWidth', 2);
xlabel('SNR (dB)'); 
ylabel('BER'); 
grid on;
