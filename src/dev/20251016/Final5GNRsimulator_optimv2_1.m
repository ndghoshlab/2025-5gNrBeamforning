%% ------------------------------------------------------------------------
% Author      : Armed Tusha (Original), Joshua (Optimized)
% Institution : University of Notre Dame
% Date        : August 1, 2025 (Original), October 6, 2025 (Optimized)
% ------------------------------------------------------------------------
...
SNRdB                             = 0:5:25;   % SNR in dB, you can input a range too, and have the results for all [0:5:20], etc.
...
% -------------------------------------------------------------------------
fprintf('Starting optimized simulation with %d iterations...\n', numIter);
...
% OPTIMIZATION: Preallocate cell arrays for precoding weights per iteration
% Will be consolidated into [numIter*noSlotsSim, numSNR] matrices after parfor
SVD_weights_cell    = cell(numIter, 1);
PMI_weights_cell    = cell(numIter, 1);
...
parfor msnr = 1:numIter
    ...
    % Preallocate temporary storage for precoding weights (per iteration)
    % Structure: [noSlotsSim, numSNR] - will store all slots for all SNR values
    SVD_weights_iter = cell(noSlotsSim, numSNR);
    PMI_weights_iter = cell(noSlotsSim, numSNR);
    
    for isi = 1:numSNR
        % OPTIMIZATION 7: Preallocate slot BER array
        ...
        for nslot = 0:noSlotsSim-1
            ...
            release(channel);               % unlock the channel
            channel.Seed = randi(1e6);      % random seed for independent realization
            ...
            %----------------------------------------------------------------------
            % Save Precoding Weights (store indexed by slot and SNR)
            %----------------------------------------------------------------------
            SVD_weights_iter{nslot + 1, isi} = newPrecodingWeight;
            if pmiPrecoding == 1 && ~isempty(fieldnames(pmiInfoPractical))
                PMI_weights_iter{nslot + 1, isi} = reshape(pmiInfoPractical.W, 1, []);
            else
                PMI_weights_iter{nslot + 1, isi} = [];
            end
        ...
        % OPTIMIZATION 9: Direct assignment for SNR results
        ber_snr(isi) = mean(ber_slot);
    end
    
    % OPTIMIZATION 10: Store results directly in preallocated matrix
    inter_snr(msnr, :) = ber_snr;
    
    % Store precoding weights for this iteration (cell contains [noSlotsSim x numSNR] structure)
    SVD_weights_cell{msnr} = SVD_weights_iter;
    PMI_weights_cell{msnr} = PMI_weights_iter;
...
%% ------------------------------------------------------------------------
% Consolidate Precoding Weights into [numIter*noSlotsSim, numSNR] matrices
% -------------------------------------------------------------------------
SVD_by_SNR = cell(1, numSNR);
PMI_by_SNR = cell(1, numSNR);

for isi = 1:numSNR
    svd_col = cell(numIter * noSlotsSim, 1);
    pmi_col = cell(numIter * noSlotsSim, 1);
    
    idx = 1;
    for msnr = 1:numIter
        svd_iter = SVD_weights_cell{msnr};
        pmi_iter = PMI_weights_cell{msnr};
        
        for nslot = 1:noSlotsSim
            svd_col{idx} = svd_iter{nslot, isi};
            pmi_col{idx} = pmi_iter{nslot, isi};
            idx = idx + 1;
        end
    end
    
    svd_col = svd_col(~cellfun(@isempty, svd_col));
    pmi_col = pmi_col(~cellfun(@isempty, pmi_col));
    
    if ~isempty(svd_col)
        SVD_by_SNR{isi} = cat(1, svd_col{:});
    else
        SVD_by_SNR{isi} = [];
    end
    
    if ~isempty(pmi_col)
        PMI_by_SNR{isi} = cat(1, pmi_col{:});
    else
        PMI_by_SNR{isi} = [];
    end
end

maxRows = max(cellfun(@(x) size(x, 1), SVD_by_SNR));
SVD_newPrecodingWeight = cell(maxRows, numSNR);
PMI_pmiInfoPractical = cell(maxRows, numSNR);

for isi = 1:numSNR
    if ~isempty(SVD_by_SNR{isi})
        for row = 1:size(SVD_by_SNR{isi}, 1)
            SVD_newPrecodingWeight{row, isi} = SVD_by_SNR{isi}(row, :);
        end
    end
    
    if ~isempty(PMI_by_SNR{isi})
        for row = 1:size(PMI_by_SNR{isi}, 1)
            PMI_pmiInfoPractical{row, isi} = PMI_by_SNR{isi}(row, :);
        end
    end
end
...
% %--------------------------------------------------------------------------
% 
% folderPath = ['C:\Users\armed\Desktop\Notre-Dame-University\5G-NR-MIMO-Project\Project-Simulator\MIMO-5G-NR-Working-Environment\FullLink\5GNRSimulator-EIRP'];
% 
% % Save consolidated weights
% % The matrices are now [numIter*noSlotsSim, numSNR] cell arrays
% % Each cell contains a row vector of weights
% 
% % Option 1: Save the complete cell arrays
% save(fullfile(folderPath, 'SVD_newPrecodingWeight.mat'), 'SVD_newPrecodingWeight', 'SNRdB', 'numIter', 'noSlotsSim');
% save(fullfile(folderPath, 'PMI_pmiInfoPractical.mat'), 'PMI_pmiInfoPractical', 'SNRdB', 'numIter', 'noSlotsSim');
% 
% % Option 2: Save each SNR column separately using your function
% % Uncomment if save_matrix_to_folder expects different format
% % for isi = 1:numSNR
% %     svd_col = SVD_newPrecodingWeight(:, isi);
% %     pmi_col = PMI_pmiInfoPractical(:, isi);
% %     
...
% %     
% %     if ~isempty(svd_col)
% %         svd_matrix = cat(1, svd_col{:});
% %         save_matrix_to_folder(svd_matrix, folderPath, ...
% %             reportConfig.PanelDimensions(1), reportConfig.PanelDimensions(2), 1);
% %     end
% %     
% %     if ~isempty(pmi_col)
% %         pmi_matrix = cat(1, pmi_col{:});
% %         save_matrix_to_folder(pmi_matrix, folderPath, ...
% %             reportConfig.PanelDimensions(1), reportConfig.PanelDimensions(2), 1);
% %     end
% % end
% 
% fprintf('\nPrecoding weights saved to folder.\n');
% 
% originalFolder = pwd;
% cd(originalFolder)
