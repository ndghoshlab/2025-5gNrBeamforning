...
noSlotsSim                        = 1;         % Number of slots per transmission, min = 1, keep max =< 100.
ModOrder                          = "256QAM";    % Modulation order QPSK, 16QAM, 64QAM, 256QAM
SNRdB                             = [20];  % SNR in dB, you can input a range too, and have the results for all [0:5:20], etc.
...
SVD_newPrecodingWeight    = [];
PMI_pmiInfoPractical      = [];
...
parfor msnr = 1:numIter
    ...
    for isi = 1:numSNR
        % OPTIMIZATION 5: Preallocate slot BER array
        ...
        for nslot = 0:noSlotsSim-1
            ...
            %----------------------------------------------------------------------
            % Save Precoding Weights
            %----------------------------------------------------------------------
            SVD_newPrecodingWeight = [SVD_newPrecodingWeight; newPrecodingWeight];
            PMI_pmiInfoPractical   = [PMI_pmiInfoPractical; reshape(pmiInfoPractical.W,1,[])];
...
save_matrix_to_folder(SVD_newPrecodingWeight,folderPath,reportConfig.PanelDimensions(1),reportConfig.PanelDimensions(2),1);
save_matrix_to_folder(PMI_pmiInfoPractical,folderPath,reportConfig.PanelDimensions(1),reportConfig.PanelDimensions(2),1);
