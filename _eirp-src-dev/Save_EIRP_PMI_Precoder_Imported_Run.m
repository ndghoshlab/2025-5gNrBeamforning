...
% -------------------------------------------------------------------------
% Imported precoder matrix
% -------------------------------------------------------------------------

folderReadData = '~/Desktop/5GNRSimulator-EIRP/Run-ICC/Joshua-Final-Results-No-PMI-Selection/TemporaryGetWeights';
dataFile = fullfile(folderReadData, ['results_20251030_004034.mat']); 
...
%NumAllPMImatricies                  = size(LoadData.matrixData,1);

ContTempPMI                         = LoadData.numIter;
%ContTempPMI                         = 1000;

%% Get the PMI vector
Imp_EIRP_PMI_Enr_AzEl_PMI_Lin      = zeros(181,361,2);
Imp_EIRP_PMI_Enr_AzEl_SSB_PMI_Lin  = zeros(181,361,2);

for pmi = 1:ContTempPMI
    pmi
    Weight_PMI_Layer                 = LoadData.SavePMIpractical_all{2,1}{pmi,9}.';      % Select pmi-th PMI matrix from the table given nLayers
    %Weight_PMI_Layer                 = LoadData.SaveSVDpractical_all{2,1}{pmi,9}.';      % Select pmi-th PMI matrix from the table given nLayers
    ...
    Imp_EIRP_PMI_Enr_AzEl_PMI_Lin(:,:,pmi)        =  ResultElAz_PMI_Lin;                        % The result is in watt scale
    Imp_EIRP_PMI_Enr_AzEl_SSB_PMI_Lin(:,:,pmi)    =  ResultElAz_PMI_Lin.*ResultElAz_SSB_Lin;    % The result is in watt scale
end

%% %%%%%%%%%%%%%%%%% Save Files for studing purposes %%%%%%%%%%%%%%%%%%% %%

folderSaveData = ['~/home/ghoshlab1/Desktop/5GNRSimulator-EIRP/Run-ICC/Joshua-Final-Results-No-PMI-Selection/Save-EIRP-Results-SVD'];

save_matrix_to_folder(Imp_EIRP_PMI_Enr_AzEl_PMI_Lin,folderSaveData,N1,N2,nLayers);

save_matrix_to_folder(Imp_EIRP_PMI_Enr_AzEl_SSB_PMI_Lin, folderSaveData,N1,N2,nLayers);
